...
public class ArrayDemo {
    ...
    /**
     * #6 Z字形变换(将一个给定字符串根据给定的行数，以从上往下、从左到右进行 Z 字形排列。比如输入字符串为 "LEETCODEISHIRING" 行数为 3 时，排列如下：)
     * L   C   I   R
     * E T O E S I I G
     * E   D   H   N
     ...
     * @return
     */
    public String convert(String str, int rowNum) {
        if (rowNum == 1) {
            return str;
        }
        List<StringBuilder> result = new ArrayList<>();
        for (int i = 0; i < Math.min(str.length(), rowNum); i++) {
        ...
        for (StringBuilder build : result) {
            finalResult.append(build.toString());
        }
